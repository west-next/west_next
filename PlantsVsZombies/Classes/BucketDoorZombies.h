...
class BucketDoorZombies :public CommonZombies
{
public:
	/* 单例模式 */
	static BucketDoorZombies* getInstance();
	void CreateBucketDoorZombies(Node* node, const Vec2& Position, const int& tag, const int& AnimationID, const bool& IsReadyScene = false);
	void CreateBucketDoorZombiesAniamton(map<int, Global::_Zombies>::iterator iter, Node* node);
	void ZombiesKilledByCar(map<int, Global::_Zombies>::iterator iter, Node* node);

private:
	static BucketDoorZombies* _instance;
